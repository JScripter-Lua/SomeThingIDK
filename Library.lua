--[[
    Vita UI Library
    Updated: Auto Scaling, Customizable Theme, Customizable Size,
             Bindable Toggle Key, Keybind Element, Notifications,
             SetTheme, Destroy -- No CoreGui
--]]
...
--[[
    ...
    Args:
        ...
        Theme      (table)          - Color overrides (see Theme fields below)

    Theme fields (all optional, Color3 values):
        Accent     - Main accent color (default: pink)
        Background - Window background
        Row        - Element row background
        RowAlt     - Alternate row / toggle off background
        Stroke     - Border stroke color
        Text       - Primary text color
        SubText    - Secondary/gradient text color
        TabBg      - Tab card background
        TabStroke  - Tab card border color
        DropBg     - Dropdown popup background
        DropStroke - Dropdown popup border
        PillBg     - Toggle pill background
--]]
function Library:Window(Args)
    ...
    local CustomSize = Args.Size      -- optional UDim2

    -- Build theme
    local uT = Args.Theme or {}
    local T = {
        Accent     = uT.Accent     or Color3.fromRGB(255, 0, 127),
        Background = uT.Background or Color3.fromRGB(11, 11, 11),
        Row        = uT.Row        or Color3.fromRGB(15, 15, 15),
        RowAlt     = uT.RowAlt     or Color3.fromRGB(10, 10, 10),
        Stroke     = uT.Stroke     or Color3.fromRGB(25, 25, 25),
        Text       = uT.Text       or Color3.fromRGB(255, 255, 255),
        SubText    = uT.SubText    or Color3.fromRGB(163, 163, 163),
        TabBg      = uT.TabBg      or Color3.fromRGB(10, 10, 10),
        TabStroke  = uT.TabStroke  or Color3.fromRGB(75, 0, 38),
        DropBg     = uT.DropBg     or Color3.fromRGB(18, 18, 18),
        DropStroke = uT.DropStroke or Color3.fromRGB(30, 30, 30),
        PillBg     = uT.PillBg     or Color3.fromRGB(11, 11, 11),
    }

    -- Track instances for runtime SetTheme
    local accentRefs = {}   -- {instance, prop}
    local bgRefs     = {}
    ...
    function Window:NewPage(Args)
        ...
        local TabStroke = Library:Create("UIStroke", { Parent = NewTabs, Color = T.TabStroke, Thickness = 1 })
        ...
        Library:Create("UICorner", { Parent = NewTabs.Banner, CornerRadius = UDim.new(0, 2) })
    ...
    --[[
        ...
        Updates theme colors at runtime.
        Pass a table with any combination of Theme fields.
        Example: Library:SetTheme({ Accent = Color3.fromRGB(0, 120, 255) })
    --]]
    function Library:SetTheme(newTheme)
        ...
        for _, ref in ipairs(accentRefs) do
            ...
            if inst and inst.Parent then
                pcall(function() inst[prop] = T.Accent end)
            end
        end
        for _, ref in ipairs(bgRefs) do
            local inst, prop = ref[1], ref[2]
            if inst and inst.Parent then
                pcall(function() inst[prop] = T.Background end)
            end
        end
    end
